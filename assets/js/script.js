var singleSlider, singleSlider2, quranSlider, weeklyTasksSlider;
var points = 0;
var tasksPoints = 0;
var checkBoxTasks;
var eatingDays = 2;
...
function updateSliderValue(slider, handle = 0) {
    ...
    while (i < children.length) {
        ...
        tasksPoints = val + parseInt(quranSlider.noUiSlider.get()) + parseInt(weeklyTasksSlider.noUiSlider.get());
        results.push(i++);
    }

    $('.contact100-form-range-value input[name="العشاءات"]').val(val);
    updatePoints();
    return results;
}
...
function updateSliderValue2(slider, handle = 0) {
    var children, i, results, val, values;
    children = slider.getElementsByClassName("noUi-handle");
    values = slider.noUiSlider.get();
    i = 0;
    ...
    while (i < children.length) {
        ...
        children[i].dataset.value = val;
        tasksPoints = val + parseInt(singleSlider.noUiSlider.get()) + parseInt(weeklyTasksSlider.noUiSlider.get());
        results.push(i++);
    }
    $('.contact100-form-range-value input[name="عدد لقاءات العشاء"]').val(val);
    updatePoints();
    return results;
}

function updateSliderValue3(slider, handle = 0) {
    var children, i, results, val, values;
    children = slider.getElementsByClassName("noUi-handle");
    values = slider.noUiSlider.get();
    i = 0;
    ...
    while (i < children.length) {
        ...
        children[i].dataset.value = val;
        tasksPoints = val + parseInt(singleSlider.noUiSlider.get()) + parseInt(quranSlider.noUiSlider.get());
        results.push(i++);
    }
    $('.contact100-form-range-value input[name="عدد لقاءات العشاء"]').val(val);
    updatePoints();
...
//----------------------------------------------------------
// var readBar = document.getElementById("read-slider");

// noUiSlider.create(readBar, {
//   connect: [true, false],
//   start: [0],
//   step: 1,
//   range: {
...
// var showReadValues = [
//   document.getElementById("read-val"),
//   // document.getElementById("value-upper")
// ];

// readBar.noUiSlider.on("update", function (values, handle) {
//   showReadValues[handle].innerHTML = Math.round(values[handle]);
//   $('.contact100-form-range-value input[name="عدد الصفحات"]').val(
//     $("#read-val").html()
//   );
// });

// //----------------------------------------------------------
// var listenBar = document.getElementById("listen-slider");

// noUiSlider.create(listenBar, {
//   connect: [true, false],
//   start: [0],
//   step: 1,
//   range: {
...
// var showlistenValues = [document.getElementById("listen-val")];

// listenBar.noUiSlider.on("update", function (values, handle) {
//   showlistenValues[handle].innerHTML = Math.round(values[handle]);
//   $('.contact100-form-range-value input[name="عدد الدقائق"]').val(
//     $("#listen-val").html()
//   );
// });
    ...
    fetch(
        ...
        .then(function(data) {
            ...
            quranSlider.noUiSlider.on("update", function() {
                return updateSliderValue2(quranSlider);
            });

            weeklyTasksSlider.noUiSlider.on("update", function() {
                return updateSliderValue3(weeklyTasksSlider);
            });

            if (sawa3edTasks.length != 0) {
                for (var i = 0; i < sawa3edTasks.length; i++) {
                    console.log($("#badercheck" + (i + 1).toString()));
...
function updatePoints() {
    ...
    $("#progressbar div").css(
        "width",
        ((points / (tasksWeight + eatingDays + numberOfWeeklyTasks + quranDaysNumber)) * 100).toString() + "%"
    );
}